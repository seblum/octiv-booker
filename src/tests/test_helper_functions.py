...
from slotbooker.helper_functions import (
    ...
    get_day_button,
    get_booking_slot,
    get_xpath_error_window,
    get_xpath_error_text_window,
...
def test_get_day():
    ...
    expected_diff_week = (
        expected_future_date.isocalendar().week - today.isocalendar().week
    )
...
@pytest.mark.parametrize(
    "booking_slot, book_action, expected_xpath",
    [
...
def test_get_xpath_error_window():
    assert (
        get_xpath_error_window()
...
def test_get_xpath_error_text_window():
    assert (
        get_xpath_error_text_window()
...
def test_continue_booking_process():
    assert continue_booking_process() is False
